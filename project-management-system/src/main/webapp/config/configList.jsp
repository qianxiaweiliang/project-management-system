    ...
    <table class="table-search">
      <tbody>
        <tr>
          ...
          <th>配置类型 0:来源配置 1:人员配置：</th>
          <td><input class="text" type="text" name="kind" value="${queryCondition.kind}"></td>
          <th>配置描述：</th>
          <td><input class="text" type="text" name="description" value="${queryCondition.description}"></td>
        </tr>
        <tr>
          <th>状态 0:启用 1:禁用：</th>
          <td><input class="text" type="text" name="status" value="${queryCondition.status}"></td>
          <th>创建人：</th>
  ...
  <table class="table table-data table-striped table-hover table-text-center">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          <a target="_blank" href="/config/detail/?id=${form.id}">查看</a>
    ...
    <tfoot>
      <tr>
        <td colspan="8">
          ...
          <div class="float-right"><liepin:pagingBar queryCondition="${queryCondition}" pageUrl="/config/list"/></div>
        </td>
      </tr>
    </tfoot>
  </table>
...
$(function(){
    ...
    vdialog.confirm('您确认要删除吗？',function(){
      $.ajax({
        url: "${pageContext.request.contextPath}/config/delete.json",
        type: "POST",
        dataType: "json",
        data: {"id":id},
