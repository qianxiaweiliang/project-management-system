...
<div class="page-content">
    ...
    <table class="table-search">
      <tbody>
        <tr>
          ...
          <th>权限名称：</th>
          <td><input class="text" type="text" name="permission_name" value="${queryCondition.permission_name}"></td>
          <th>备注：</th>
  ...
  <table class="table table-data table-striped table-hover table-text-center">
    ...
    <tbody>
      ...
      <tr>
        <td>${form.id}</td>
        <td>${form.permissionName}</td>
        <td>${form.memo}</td>
        <td>${form.creatorId}</td>
        <td>${form.createtime}</td>
        <td>${form.modifytime}</td>
        <td>
          <a target="_blank" href="/permission/detail/?id=${form.id}">查看</a>
    ...
    <tfoot>
      <tr>
        <td colspan="7">
          ...
          <div class="float-right"><liepin:pagingBar queryCondition="${queryCondition}" pageUrl="/permission/list"/></div>
        </td>
      </tr>
    </tfoot>
  </table>
...
$(function(){
    ...
    vdialog.confirm('您确认要删除吗？',function(){
      $.ajax({
        url: "${pageContext.request.contextPath}/permission/delete.json",
        type: "POST",
        dataType: "json",
        data: {"id":id},
