...
<div class="page-content">
  ...
  <form action="/rolepermission/list/" method="post">
    <table class="table-search">
      <tbody>
        <tr>
          ...
          <th>权限id：</th>
          <td><input class="text" type="text" name="permission_id" value="${queryCondition.permission_id}"></td>
        </tr>
        <tr>
          <th>创建人：</th>
  ...
  <table class="table table-data table-striped table-hover table-text-center">
    ...
    <tbody>
      ...
      <tr>
        <td>${form.id}</td>
        <td>${form.roleId}</td>
        <td>${form.permissionId}</td>
        <td>${form.creatorId}</td>
        <td>${form.createtime}</td>
        <td>${form.modifytime}</td>
        <td>
          <a target="_blank" href="/rolepermission/detail/?id=${form.id}">查看</a>
    ...
    <tfoot>
      <tr>
        <td colspan="7">
          ...
          <div class="float-right"><liepin:pagingBar queryCondition="${queryCondition}" pageUrl="/rolepermission/list"/></div>
        </td>
      </tr>
    </tfoot>
  </table>
...
$(function(){
    ...
    vdialog.confirm('您确认要删除吗？',function(){
      $.ajax({
        url: "${pageContext.request.contextPath}/rolepermission/delete.json",
        type: "POST",
        dataType: "json",
        data: {"id":id},
  ...
  $('[data-selector="edit"]').click(function(){
    var id = $(this).data('id');
    var url = "/rolepermission/edit/?id="+id;
  ...
  $('[data-selector="add"]').click(function(){
    var url = "/rolepermission/edit/?id=";
    window.location.href=url;
    //$.dialog.load(url).lock();
    return false;
  });
